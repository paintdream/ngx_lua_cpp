...
namespace iris {
	...
	struct ngx_hooker_t {
		...
		ngx_hooker_t() {
			...
			ngx_pending_events.prev = &ngx_pending_events;
			ngx_pending_events.next = &ngx_pending_events;
		...
		void registar(iris_lua_t lua) {
			const std::string_view wrapper = 
				...
				"	return function (...)\n"
				"		func(...)\n"
				"		return get_coroutine_returns()\n"
				"	end\n"
				"end\n";
			lua.set_global("__ngx_iris_wrap_coroutine_with_returns__", lua.call<lua_t::ref_t>(lua.load(wrapper, "=(ngx_lua_cpp)"), &get_coroutine_returns));
	...
	private:
		...
		ngx_int_t process_events(ngx_cycle_t* cycle, ngx_msec_t timer, ngx_uint_t flags) {
			...
			if (offset_http_co_ctx_event_queue != 0) {
				for (auto* co_ctx : pending_lua_http_co_ctxs) {
					...
					ngx_queue_remove(p);
					ngx_queue_insert_tail(&ngx_pending_events, p);
				}
			}
		...
		ngx_queue_t ngx_pending_events = { nullptr, nullptr };
	};
	...
	void ngx_lua_cpp_t::process_events() {
		if (async_worker.get_thread_count() <= 1) {
